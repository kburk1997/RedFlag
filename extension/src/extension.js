...
function getTarget() {
  chrome.tabs.query({ currentWindow: true, active: true }, function(tabs) {
    chrome.tabs.sendMessage(tabs[0].id, "getText", sentiment);
  });
}
	...
	if(message_holder != null && message_holder!=undefined){
		...
		if (profane==true){
			...
			chrome.storage.sync.get({
			    ...
			    var mode = items.appMode;

			    if (mode=='hide'){
			    	$(tweet).hide();
			    	$(tweet).parent().append("<h3 style=\"color: #f66\">This tweet was hidden because it may be harassment.</h3><br><p>Click here to view the offending tweet.</p>");
			    	return;
			    }
			    else{
			    	$(tweet).css("background-color","#fff6f6");
					...
					//Check if a user was 
					var table="<table><tr><td><h3 style=\"color: #f66\">This tweet might be harassment</h3></td><td><a href=\"http://s.codepen.io/kburk97/debug/woGqPM\">Your Options</a></td></tr></table>";
					$(tweet).append(table);
					return;			    	
			    }
			  });
